...
if(file_exists(temp_file)){untar(tarfile = temp_file, exdir = extract_to_dir)}
```
...
## The resulting data 
The extracted data comprises an unarchived file named `swda`, which includes one HTML file located in the `doc` folder, along with 14 folders containing `.utt` files.
